...
echo "Starting SITL..."
# Run WITHOUT --map and --console to avoid X11 crashes if run headless, but here user wants UI so it's fine.
...
echo "Starting Pipeline with Live UI and Video..."
DRY_RUN=0 TELEMETRY_IN_PORT=14550 ./venv/bin/python -m src.main --mode sitl --show-ui --eval-video youtube_raw.mp4
